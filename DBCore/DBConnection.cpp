...
namespace FrokEngine
{
	...
	{
		SQLULEN strLen = static_cast<SQLULEN>((::wcslen(str) + 1) * 2);
		*index = SQL_NTSL;

		if (strLen > WVARCHAR_MAX)
			return BindParam(paramIndex, SQL_C_WCHAR, SQL_WLONGVARCHAR, strLen, (SQLPOINTER)str, index);
		else
			return BindParam(paramIndex, SQL_C_WCHAR, SQL_WVARCHAR, strLen, (SQLPOINTER)str, index);
	}
